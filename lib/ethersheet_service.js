...
const SheetCollection = require('./models/sheet_collection');

/***********************************************
 * EtherSheetService
 ***********************************************/
let EtherSheetService = module.exports = function(config){
  let es = this;
  es.config = config;
...
// EtherSheet API
EtherSheetService.prototype.refreshDbConnection = function(callback)
{
    let es = this;
    es.db = new ueberDB.database(
        es.config.db_type, {
...
EtherSheetService.prototype.getSheetCollectionIds = function(collection_id,cb) {
  let es = this;
  let sheet_collection = new SheetCollection(collection_id,es,es.db);
...
EtherSheetService.prototype.getSheetHeaders = function (sheet_id, cb) {
    let es = this;
    es.getSheet(sheet_id,function(err,sheet_data) {
        let lcol, cell, id, frow = sheet_data.rows[0];
        let headers = [];
        ...
        for (lcol = 0; lcol < sheet_data.cols.length; lcol++) {
            if (_.isUndefined(sheet_data.cells[frow]) || _.isUndefined(sheet_data.cells[frow][sheet_data.cols[lcol]]))
                ...
                id   = frow + "-" + sheet_data.rows[lcol];
                cell = sheet_data.cells[frow][sheet_data.cols[lcol]].value.toString().trim();
                headers.push({name: cell, id: id})
            }
        }
        cb(JSON.stringify(headers), null);
    })
};
...
EtherSheetService.prototype.createSheetFromCSV = function(sheet_id,data,cb){
  ...
  let colmap = {};
  let csv_parser = csv();
  let sniffer = new CSVSniffer([';',',','\t']);
  let sniffResult = sniffer.sniff(data.toString(), {quoteChar:'"', hasHeader: true});
  let delimiter = (sniffResult.delimiter === null) ? es.guessDelimiter(data.toString()) : sniffResult.delimiter;
  if(delimiter !== null)
  ...
  csv_parser.from(data.toString())
      .transform( function(row, row_id){
        ...
        for(let col_idx = 0; col_idx < sheet.cols.length; col_idx++){
          let col_uuid = colmap[col_idx];
          let cell = {value:row[col_idx], type:Sheet.getCellType(row[col_idx])};
      ...
      /*  for(let i = 0; i < es.config.default_row_count; i++){
          sheet.rows.push(uuid());
        }
        for(let i = 0; i < es.config.default_col_count; i++)
          sheet.cols.push(uuid());*/

        es.deleteSheet(sheet_id, function(){
          es.createSheet(sheet_id, sheet, cb, es);
        });
      })
...
EtherSheetService.prototype.createSheet = function(sheet_id,data,cb, es){

  if(_.isUndefined(es)) es = this;
